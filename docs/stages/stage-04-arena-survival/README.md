# 阶段 04：Arena Survival：从“动起来”转向“规则与判断”

把小规模战斗、敌人 AI、资源管理和 survive loop 组合起来。

## 省流版

- 生存玩法测试系统分层和设计边界。
- 剩余生命值、敌人生成、分数和状态需要清晰归属。
- 复杂玩法往往由多个小系统叠加而成。
- 一个良好的 arena loop 让你能稳步迭代。
...
比如，阶段 04：Arena Survival：从“动起来”转向“规则与判断” 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
```rust
if enemy_hp <= 0 { despawn_enemy(); score += 10; }
```
