# 阶段 01：Spore Swarm：让系统从简单局部行为逐步扩展

建立一批小型实体网络，演示扩散、跟随、相近检测和环境响应。

## 省流版

- 小系统可以组合成复杂行为。
- 相邻实体和群体模式需要清晰的状态定义。
- 群体模拟比单个实体更需要对边界进行审慎设计。
- 真实世界的复杂性经常来自大量小规则叠加。
...
比如，阶段 01：Spore Swarm：让系统从简单局部行为逐步扩展 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
```rust
for nearby in query.iter() { if dist < threshold { steer_toward(); } }
```
