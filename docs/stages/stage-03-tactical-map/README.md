# 阶段 03：Tactical Map：空间布局和信息组织的关键
...
- 地图不是背景，而是信息结构。
- 多个单位共享地面标识、遮挡和覆盖信息。
- 系统必须理解世界构造，而不是只是“点一下就移动”。
- 地图组织方式决定后续 AI 和战斗逻辑的复杂度。
...
比如，阶段 03：Tactical Map：空间布局和信息组织的关键 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
```rust
cell.occupied = true;
if can_move_to(cell) { path.push(cell); }
```
