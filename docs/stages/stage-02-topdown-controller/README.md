# 阶段 02：Top-down Controller：把输入转成可读、可重用的移动策略

指导如何建模二维横向移动、速度、方向和动作分离。
...
比如，阶段 02：Top-down Controller：把输入转成可读、可重用的移动策略 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
```rust
let move_dir = input_vector.normalize_or_zero();
