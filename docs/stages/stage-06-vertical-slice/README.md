# 阶段 06：Vertical Slice：完整体验的工程收束

把前前后后所有系统都纳入一份可演示的游戏垂直切片，验证产品闭环。

## 省流版

- 垂直切片不是“所有功能都塞进一个原型”，而是“最关键体验被完整验证”。
- 工程成熟度体现在系统边界、保存、反馈和调试都能工作。
- 一次不够完整的 slice 可能暴露出设计上的深层问题。
- 完成垂直切片，说明你不是在拼积木，而是在创建完整体验。
...
比如，阶段 06：Vertical Slice：完整体验的工程收束 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
```rust
run_demo_loop();
assert_gameplay_loop_is_stable();
```
