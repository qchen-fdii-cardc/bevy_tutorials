# Rust in Bevy：Performance Profiling：先测量，再微调
...
讲述 profiler、run-time cost and hot path 的思路，帮助工程界限与性能分析。
...
1. 性能问题通常不是“没有优化”，而是“看不见真实瓶颈”。

2. 光看帧率不够，必须看系统耗时与组件查询成本。

3. profile 之前，先定义被观察对象和观察维度。

4. 真实的优化是让系统边界更稳定，而不是临时 hack。
