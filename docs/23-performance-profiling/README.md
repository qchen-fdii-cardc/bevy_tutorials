# Bevy 教程 23：Performance Profiling：先测量，再微调

讲述 profiler、run-time cost and hot path 的思路，帮助工程界限与性能分析。

## 省流版

- 性能问题通常不是“没有优化”，而是“看不见真实瓶颈”。
- 光看帧率不够，必须看系统耗时与组件查询成本。
- profile 之前，先定义被观察对象和观察维度。
- 真实的优化是让系统边界更稳定，而不是临时 hack。
...
比如，Bevy 教程 23：Performance Profiling：先测量，再微调 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
