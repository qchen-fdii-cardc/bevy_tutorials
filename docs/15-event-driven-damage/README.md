# Bevy 教程 15：Event-Driven Damage：伤害应当是事实，而不是一条隐式公式

把伤害从“直接扣血”升级为“事件 -> 处理 -> 状态更新”的明确链路。

## 省流版

- 事件记录的是“谁攻击了谁”。
- 伤害计算更适合拆成独立系统，再在下一步写回 Health。
- 事件驱动有利于扩展，例如火焰、击退、暴击等。
- 如果直接写 `health -= x`，你会发现规则很难复用。
...
比如，Bevy 教程 15：Event-Driven Damage：伤害应当是事实，而不是一条隐式公式 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
app.add_systems(Update, apply_damage_from_events);
app.add_systems(Update, resolve_death);
```
