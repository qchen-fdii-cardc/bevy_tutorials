...
- AssetServer 是加载与查询入口。
- Handle<T> 是引用，不是数据本身。
- LoadState 表示 Loading/Loaded/Failed 等真实阶段。
- 异步加载的关键在于把状态表达清楚，而不是盯着屏幕等待。
...
比如，Bevy 教程 17：Asset Server：资源句柄、加载状态与异步边界 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
