# Bevy 教程 02：Debug Visualization：日志、可视化和状态证明
...
- 日志最小，却是验证世界状态最直接的工具。
- 可视化要服务于假设，而不是只为了炫技。
- Bevy 中的 gizmos、debug log 与断言同样重要。
- 如果你不能证明状态变了，视觉上的“像对”不算结论。
...
比如，Bevy 教程 02：Debug Visualization：日志、可视化和状态证明 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
```rust
println!("player hp = {}", player_hp.0);
app.add_systems(Update, draw_bounds);
```
