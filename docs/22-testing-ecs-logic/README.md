# Bevy 教程 22：Testing ECS Logic：把世界当成测试对象
...
比如，Bevy 教程 22：Testing ECS Logic：把世界当成测试对象 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
