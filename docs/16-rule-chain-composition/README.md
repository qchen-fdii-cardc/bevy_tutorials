# Bevy 教程 16：Rule Chain Composition：从一次攻击到一条规则链

说明为什么一整个 `match` 不如显式系统链清晰：规则链把状态变化分成多个可验证步骤。

## 省流版

- 攻击并不只是一个数字，它是发起者、基础威力、防御和最终生命值的组合。
- 一条规则链让每步都有明确输入和输出。
- 顺序是设计的一部分，而不是实现细节。
- 规则链本质上是 ECS 中的“状态流”。
...
比如，Bevy 教程 16：Rule Chain Composition：从一次攻击到一条规则链 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
```rust
app.add_systems(Update, (begin_attack, apply_armor_reduction, apply_health_change, resolve_death).chain());
```
