# Bevy 教程 12：Sprites, Layers 与 Z：图层不是玄学，而是深度和顺序

解释 Z 轴、渲染层和 sprite ordering 在可视化中的意义。

## 省流版

- 图层常常由 Z 和 sorting 来表达。
- 渲染顺序不是“随便看起来像对”，而是模型的一部分。
- 多个实体共享同一平面时，排序规则决定谁在前谁在后。
- 层级设计与世界设计一样重要。
...
比如，Bevy 教程 12：Sprites, Layers 与 Z：图层不是玄学，而是深度和顺序 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
