# Bevy 教程 21：Plugin Project Structure：把代码拆成模块，给项目留出可扩展的结构

解释插件、模块和层次结构如何让工程管理从“小而乱”成长为“可维护”。
...
比如，Bevy 教程 21：Plugin Project Structure：把代码拆成模块，给项目留出可扩展的结构 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
```rust
struct CombatPlugin;
impl Plugin for CombatPlugin { fn build(&self, app: &mut App) { app.add_systems(Update, resolve_hit); } }
```
