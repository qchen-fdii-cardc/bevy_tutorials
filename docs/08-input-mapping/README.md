# Bevy 教程 08：Input Mapping：把设备事件翻译成游戏动作

把用户输入从底层按键事件提升为角色动作、动作队列和状态变更。

## 省流版

- 输入层负责“键盘、手柄、触屏事件”这种原始数据。
- 映射层负责把它变成“攻击、转身、跳跃”这样的游戏动作。
- 不做映射，代码会直接绑定设备细节。
- 好的动作层更容易扩展到不同控制器。
...
比如，Bevy 教程 08：Input Mapping：把设备事件翻译成游戏动作 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
```rust
enum PlayerAction { Move, Jump, Attack }
let action = if input.just_pressed(KeyCode::Space) { PlayerAction::Jump } else { ... };
```
