# Bevy 教程 19：Audio 与 UI Feedback：反馈是游戏的解释层

说明声音、动画和 UI 等反馈层如何作为“解释游戏状态”的手段出现。

## 省流版

- 反馈系统告诉玩家“发生了什么”。
- UI 和音效不只是视觉装饰，它们同样是状态的可见化。
- 反馈越强，越要保证它读取的是真实状态，而不是猜测。
- 把反馈做成系统，容易测试和调整。
...
比如，Bevy 教程 19：Audio 与 UI Feedback：反馈是游戏的解释层 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
