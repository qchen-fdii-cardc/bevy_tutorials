# Bevy 教程 01：Plugin、Resource 与 System：把 App 拆成模块

解释 Plugin 如何组织时序，Resource 如何承载全局状态，System 如何在世界中做数据变换。

## 省流版

- Plugin 是带状态的注册单元，不只是语法糖。
- Resource 是跨系统共享的真实状态，而不是传参的“全局变量”。
- System 依赖的参数表明它读取了什么、修改了什么。
- ECS 的优点在于边界清晰：不同逻辑不抢同一块状态。
...
比如，Bevy 教程 01：Plugin、Resource 与 System：把 App 拆成模块 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
下一章开始讨论调试：在不知道状态为什么变化时，最好的工具不是猜，而是可视化和日志。
