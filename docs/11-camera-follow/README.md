# Bevy 教程 11：Camera Follow：相机不是“附着在玩家上”，而是一个独立的空间策略

讲相机平滑跟随、视野、世界空间和屏幕空间的边界。

## 省流版

- 相机是另一层空间的视角。
- 跟随是一个空间策略，不等于把玩家的 transform 复制过去。
- 你需要区分世界坐标、相机坐标与屏幕坐标。
- 平滑跟随最容易产生 lag 或 overshoot，必须设定边界。
...
比如，Bevy 教程 11：Camera Follow：相机不是“附着在玩家上”，而是一个独立的空间策略 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
```rust
camera.translation = camera.translation.lerp(player_pos.extend(10.0), 0.1);
```
