# Bevy 教程 10：2D Coordinates：你需要知道空间和方向的真实语义

从坐标系、向量和 `Transform` 的关系出发，解释 2D 空间中“方向”和“位置”如何被表示。

## 省流版

- 2D 不是把 3D 简单压成两个轴，而是有自己的语义和相对关系。
- 变换、方向和速度必须分开看。
- 很多“移动不对”问题，最终在于坐标系或旋转理解错误。
- 清晰的空间语义，能减少大量“鬼畜运动”的调试成本。
...
比如，Bevy 教程 10：2D Coordinates：你需要知道空间和方向的真实语义 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
