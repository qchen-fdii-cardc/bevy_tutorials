# Bevy 教程 09：State 与 Run Condition：让系统在正确时刻生效

说明状态机和 run condition 的区别：状态描述“当前在哪个世界阶段”，条件描述“是否该运行某条逻辑”。

## 省流版

- State 适合表达当前游戏阶段。
- Run condition 适合门控一条系统。
- 状态与条件不一定一一对应。
- 真正稳定的系统依赖，是“可被测试证明的门控”。
...
比如，Bevy 教程 09：State 与 Run Condition：让系统在正确时刻生效 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
