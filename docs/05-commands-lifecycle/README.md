# Bevy 教程 05：Commands 与 Lifecycle：实体如何出现、更新与销毁

覆盖 Commands 和 entity lifecycle，解释事件顺序和命令缓冲区的设计意图。

## 省流版

- Commands 并不是立即修改世界，而是队列一批改变。
- 实体的诞生与销毁并非同一时间点发生。
- 理解生命周期，才能写出不出现“删了又用”的系统。
- 调度器对命令的处理顺序决定了你写的代码是否稳定。
...
比如，Bevy 教程 05：Commands 与 Lifecycle：实体如何出现、更新与销毁 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
