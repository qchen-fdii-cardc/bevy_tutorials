# Bevy 教程 24：Save Settings 与 Release：在产品状态下做工程决策

把工程成熟度从“能跑”提升到“可配置、可保存、可持续交付”。

## 省流版

- 保存配置和设置，是使游戏可持续运营的工程基础。
- 发布不只是构建产物，而是调试、配置和版本边界的集合。
- 设置保存通常依赖世界状态和资源的一致性。
- 成熟项目把开发流程和运行时行为视为同一套系统。
...
比如，Bevy 教程 24：Save Settings 与 Release：在产品状态下做工程决策 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
let bytes = ron::ser::to_string(&settings).unwrap();
```
