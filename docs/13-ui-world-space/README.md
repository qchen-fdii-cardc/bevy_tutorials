# Bevy 教程 13：UI 与 World Space：屏幕坐标与世界坐标的边界

讲解 UI 组件与世界空间实体之间的区分，避免“一个 Transform 处理所有场景”的错误。

## 省流版

- UI 是屏幕空间的布局系统。
- 世界空间实体是场景中的真实位置。
- 把 UI 和 World 做混合布局，通常是问题的根源。
- 定位工具要清楚自己是测屏幕还是测世界。
...
比如，Bevy 教程 13：UI 与 World Space：屏幕坐标与世界坐标的边界 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
