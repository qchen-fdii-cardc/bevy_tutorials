# Bevy 教程 06：Event 与 Change Detection：状态变化的证据链

讲一个关键问题：变化检测和事件不是一回事，前者是观察对比，后者是记录发生过什么。

## 省流版

- 事件适合记录“发生过怎样的事实”。
- Change detection 更关注“这个值和上一帧有什么不同”。
- 只靠视觉很难判断状态变化是否真实。
- 事件系统帮助你把因果关系记录成可靠的输出。
...
比如，Bevy 教程 06：Event 与 Change Detection：状态变化的证据链 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
app.add_systems(Update, fire_damage_event);
app.add_systems(Update, react_to_damage);
```
...
下一章会继续把时序问题具体化：固定时间步与帧更新的区别是什么。
