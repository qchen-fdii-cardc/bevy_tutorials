# Bevy 教程 18：Animation State：动画不是图片播放，而是状态机

讲述 sprite animation、state machine 与 transition 的关系。

## 省流版

- 动画状态描述角色的行为阶段，而不是单纯的图像序列。
- 状态转换必须是显式的，否则很容易出现瞎切换。
- 动画和逻辑需要彼此隔离，但同样依赖状态。
- 状态机是控制角色可见行为的抽象。
...
比如，Bevy 教程 18：Animation State：动画不是图片播放，而是状态机 这种主题，常见错误是把状态和行为捆在一起：一个结构体里混着输入、输出、生命周期和显示状态；或者把世界的变化用 `if` 穿成一大串，不再界定系统边界。这会让调试越来越难，因为你无法知道“哪条规则写进了什么”。
...
app.add_systems(Update, advance_animation_state.run_if(in_state(AnimState::Run)));
```
